...
The ``ratelimit`` and ``ratelimit_interval`` options, respectively an integer and a decimal
number, control the number of Web service requests per interval of time (default: 1 request
per second). **Do not change the rate limit setting** if you're
...
As other plugins may send requests to the MusicBrainz API, the following ``musicbrainz``
options will also modify the other plugins' connection to MusicBrainz, even if the
``musicbrainz`` plugin is disabled:
- ``host``
- ``https``
- ``ratelimit``
- ``ratelimit_interval``
